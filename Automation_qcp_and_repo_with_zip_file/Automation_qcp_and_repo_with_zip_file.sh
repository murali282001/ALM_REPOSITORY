        ...
        *)
            ...
            if [[ "$Download" == "YES" ]]; then
                ...
                if [[ -d "$folder_name" ]]; then
                    log_warning "Folder $folder_name already exists. Skipping folder creation and download the zip or 7za or tar file and Extraction of the zip files."
                    continue
                else
                    log_info "Creating new folder for safty perpose: $folder_name"
                    if sudo mkdir -p "$folder_name"; then
                         sudo chown alm_service:'domain users' "$folder_name"
                         sudo chmod 775 "$folder_name"
                         log_info "Folder $folder_name created with correct ownership and permissions."
                    else
